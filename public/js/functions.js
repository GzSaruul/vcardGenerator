...
export function buildVCard() { // Renamed saveData to buildVCard and exported it
  const $languageSelect = document.getElementById("languageSelect");
  const getValue = (name) => {
    const $data = Array.from(document.querySelectorAll(".data__input"));
    const i = $data.findIndex((input) => input.name === name);
  ...
  if (lang === "MN") {
    return [
      ...
      `ADR;TYPE=WORK;CHARSET=UTF-8:${getValue("POBox_MN")};;${getValue("Street_MN")};${getValue("City_MN")};
      ${getValue("Region_MN")};${getValue("Postal_MN")};${getValue("Country_MN")}`,
    ...
    return [
      ...
      `ADR;TYPE=WORK:${getValue("POBox_EN")};;${getValue("Street_EN")};${getValue("City_EN")};
      ${getValue("Region_EN")};${getValue("Postal_EN")};${getValue("Country_EN")}`,
      `TEL;TYPE=CELL:${getValue("Phone")}`, `EMAIL;TYPE=INTERNET:${getValue("Email")}`,
      `URL:${getValue("Website")}`, "END:VCARD"
    ].join("\r\n");
  }
...
async function callSaveEmployeeAPI() {
  ...
  // Use getValue to ensure we get the value from the input fields
...
export function updateQR() {
  ...
  QRCode.toCanvas(canvas, vcard, { width: 280, height: 280, errorCorrectionLevel: "H" },
    err => {
      if (err) {
...
// ─── QR Generation and Data Saving ────────────────────────────────
export async function generateQR(){
  updateQR();
  await callSaveEmployeeAPI(); // Attempt to save the employee data
...
export function downloadQR(){
    if (!$qrContainer) {
        displayMessage("QR container not found.", "error");
        return;
    }
    const canvas = $qrContainer.querySelector("canvas");
    if(!canvas) {
        displayMessage("Татах QR код олдсонгүй. Эхлээд үүсгэнэ үү.", "error");
        return;
    }
    const extElement = document.querySelector("select[name='extension']");
    const ext = extElement ? extElement.value : "png"; // Default to png if select not found
    ...
    if (lang === "MN") {
        firstName = getValue("Name_MN");
        lastName = getValue("LastName_MN");
    } else {
        firstName = getValue("Name_EN");
        lastName = getValue("LastName_EN");
    }
    ...
    if (sanitizedFirstName) {
        filename += `_${sanitizedFirstName}`;
    }
    if (sanitizedLastName) {
        filename += `_${sanitizedLastName}`;
    }
    filename += `.${ext}`;
    ...
    try {
        ...
        displayMessage(`Татаж авахад алдаа гарлаа. (${ext}) форматыг дэмжихгүй байж магадгүй.`, "error");
    }
}

// ─── Event Listeners ─────────────────────────────────────────────
$data.forEach(el => {
    if(el) el.addEventListener("input", () => checkData(el));
});

if ($languageSelect) {
    $languageSelect.addEventListener("change", () => {
        updateQR();
        // $data.forEach(input => { if(input) checkData(input); }); // Removed this line
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    $data.forEach(input => { if(input) checkData(input); });
    updateQR();
    // Set initial message if generate button is disabled (meaning form is empty or invalid)
    if ($buttonGenerate && $buttonGenerate.disabled) {
        const hasErrors = Object.values(inputError).includes(false);
        if (hasErrors) {
            displayMessage("Утгуудыг үнэн зөв оруулна уу.", "error");
        } else {
            displayMessage("Бүх утгыг бүрэн бөглөнө үү.", "info");
        }
    } else if ($buttonGenerate) {
         displayMessage("Ready to generate and save.", "info");
    }
});
export default { checkData, buildVCard, updateQR, generateQR, downloadQR, callSaveEmployeeAPI }; // Updated export